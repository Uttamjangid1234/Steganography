#correct for saving the encoded image with different name
from tkinter import *
...
class IMG_Stegno:
    ...
    def main(self, root):
        ...
        title = Label(frame, text='Image Obfuscated Messaging')
        title.config (font=('Times new roman', 25, 'bold'), bg= '#87CEEB')
        title.grid(pady=10)
        title.grid(row=1)

        encode = Button(frame, text="Encode", command=lambda: self.encode_frame1(frame), padx=14, bg='#90EE90')
        encode.config(font=('Helvetica', 14), bg='#90EE90')
        encode.grid(row=2) 
        decode = Button(frame, text="Decode", command=lambda: self.decode_frame1(frame), padx=14, bg='#90EE90')
        decode.config(font=('Helvetica', 14), bg='#90EE90')
        decode.grid(pady=12)
        decode.grid(row=3)
    ...
    def encode_frame1(self, F):
        ...
        label1.config(font=('Times new roman', 25, 'bold'), bg='#87CEEB')
        label1.grid()

        button_bws = Button(F2, text='Select', command=lambda: self.encode_frame2(F2))
        button_bws.config(font=('Helvetica', 18), bg='#90EE90')
        button_bws.grid()
        button_back = Button(F2, text='Cancel', command=lambda: IMG_Stegno.back(self, F2))
        button_back.config(font=('Helvetica', 18), bg='#FF7F7F')
    ...
    def decode_frame2(self, d_F2):
        ...
        if not myfiles:
            ...
            button_back.config(font=('Helvetica', 14), bg='#FF7F7F')
            button_back.grid(pady=15)
            d_F3.grid(row=1)
            d_F2.destroy()

    # DataFlair- function to decode data
